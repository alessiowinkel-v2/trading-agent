...
## 2026-09-13 — Citing a prior lesson's conclusion is a distinct failure mode from mis-deriving a fact fresh

**Context**: The 2026-09-06 LESSONS.md entry conclusively resolved the root-level `PAUSED.flag` as tracked in git (commit `5db43a1a`, dated 2026-07-30). The 2026-09-11 research log entry cited that same 2026-09-06 conclusion but described it as resolving to "untracked-or-incidental debris" — inverting the actual finding while presenting it as settled. This weekly review re-verified fresh (`git ls-files --error-unmatch PAUSED.flag` exits 0; `git status --porcelain PAUSED.flag` shows no changes) and confirms: tracked, clean, content "test", unchanged.
**Insight**: The standing instruction to "re-derive fresh from source, don't paraphrase" (2026-07-24, 2026-07-31 lessons) was aimed at git-log derivations going forward. It did not cover the case of citing a *previous* lesson's already-settled conclusion — that citation can itself invert the fact while sounding authoritative ("per the 2026-09-06 lesson, resolved as..."). A reader trusts a citation more than a fresh claim, making this error class more dangerous, not less.
**Rule change**: Flag for user: when citing a prior LESSONS.md or WEEKLY-REVIEW.md conclusion, quote the relevant sentence directly rather than summarizing it from memory. No unilateral strategy change.

## 2026-09-13 — Sustained multi-day pressure is a harder test of the fragile-premium rule than any single day
